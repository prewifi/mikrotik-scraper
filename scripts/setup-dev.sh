...
docker compose -f compose.dev.yaml exec mikrotik-scraper-dev \
  bash -c "git config --global user.email 'dev@mikrotik-scraper.local' && git config --global user.name 'Development User'"
...
echo "1. Access the container: docker compose -f compose.dev.yaml exec mikrotik-scraper-dev bash"
